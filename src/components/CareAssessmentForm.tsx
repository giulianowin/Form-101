...
const REQUIRED_FIELDS_BY_STEP = {
  1: ['firstName', 'lastName', 'dateOfBirth', 'phoneNumber', 'gender', 'address', 'postcode', 'allergies', 'serviceRequired'],
  2: ['fullName', 'relationshipToClient', 'phoneNumber', 'email', 'address', 'postcode'],
  3: ['medicalHistory', 'currentDiagnosis', 'dnarInPlace', 'requiresHelpWithAppointments', 'wantsCompanyToAppointments'],
  4: ['consent']
};
...
const CareAssessmentForm: React.FC = () => {
  ...
  const isCurrentSectionValid = (step: number): boolean => {
    ...
    switch (step) {
      case 1:
        return requiredFields.every(field => {
          const value = serviceUserDetails[field as keyof ServiceUserDetails];
          return value && value.trim() !== '';
        });
      case 2:
        return requiredFields.every(field => {
          const value = nextOfKinDetails[field as keyof NextOfKinDetails];
          return value && value.trim() !== '';
        });
      case 3:
        return requiredFields.every(field => {
          const value = medicalBackgroundInformation[field as keyof MedicalBackgroundInformation];
          return value && value.trim() !== '';
        });
      case 4:
        return consent;
  ...
  // Check if a step is accessible (current step or previous steps are completed)
  const isStepAccessible = (step: number): boolean => {
    if (step === 1) return true;
    
    // Check if all previous steps are completed
    for (let i = 1; i < step; i++) {
      if (!isCurrentSectionValid(i)) {
        return false;
      }
    }
    return true;
    ...
    try {
      const submissionData = {
        serviceUserDetails: {
          ...
          clientStartDate: serviceUserDetails.clientStartDate || 'N/A',
        ...
        medicalBackgroundInformation: {
          ...
          hospitalAdmissionHistory: medicalBackgroundInformation.hospitalAdmissionHistory || 'N/A',
          mobilitySupport: medicalBackgroundInformation.mobilitySupport || 'N/A',
          skinIntegrityNeeds: medicalBackgroundInformation.skinIntegrityNeeds || 'N/A',
          dnarInPlace: medicalBackgroundInformation.dnarInPlace,
          careVisitFrequency: medicalBackgroundInformation.careVisitFrequency || 'N/A',
          careVisitDuration: medicalBackgroundInformation.careVisitDuration || 'N/A',
  ...
  const nextStep = () => {
    if (currentStep < 4 && isCurrentSectionValid(currentStep)) {
          ...
          <div key={step} className="flex items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors
              ${currentStep === step 
                ...
                : isCurrentSectionValid(step)
                ? 'bg-green-500 text-white'
                : isStepAccessible(step)
                ? 'bg-gray-300 text-gray-600'
                : 'bg-gray-100 text-gray-400'
              }`}>
              {isCurrentSectionValid(step) && currentStep !== step ? (
            ...
            {step < 4 && (
              <div className={`h-1 w-24 mx-2 transition-colors ${
                isCurrentSectionValid(step) ? 'bg-green-500' : 'bg-gray-200'
              }`} />
            )}
          </div>
        ))}
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-8">
            ...
            <form onSubmit={handleSubmit}>
              ...
              {currentStep === 2 && isStepAccessible(2) && renderStep2()}
              {currentStep === 3 && isStepAccessible(3) && renderStep3()}
              {currentStep === 4 && isStepAccessible(4) && renderStep4()}
              ...
              <div className="flex justify-between mt-8">
                ...
                {currentStep < 4 ? (
                  <button
                    type="button"
                    onClick={nextStep}
                    disabled={!isCurrentSectionValid(currentStep)}
                    className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors ml-auto"
                  >
                    Next
                  </button>
                ) : (
                  <button
                    type="submit"
                    disabled={!isCurrentSectionValid(3) || !consent || isSubmitting}
